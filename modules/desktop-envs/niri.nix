  ...
  flake.modules.homeManager.base =
    ...
    let
      ...
      outputsKdl = lib.concatStringsSep "\n" (
        lib.mapAttrsToList mkMonitorKdl (config.local.host.monitors.niri or { })
      );
    in
    {
          ...
          "systemd/user/niri.service.d/override.conf".text =
            let
              envList = lib.mapAttrsToList (name: value: "Environment=${name}=${value}") (
                config.local.host.gpu.offloadVars or { }
              );
              offloadLinesRaw = lib.concatStringsSep "\n" envList;
              offloadLines = if (config.local.host.gpu.niri.enable or false) then offloadLinesRaw else "";
            in
            # toml
            ''
              [Service]
